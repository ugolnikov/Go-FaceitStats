...
function extractSteamId(input: string): string | null {
	...
	try {
		...
		if (
			...
			url.hostname.endsWith('.steamcommunity.com')
		) {
...
async function resolveVanityUrlToSteamId(
	vanity: string
): Promise<string | null> {
...
async function getPlayerStats(playerId: string, matchesLimit?: number) {
	...
	if (matchesLimit) {
		try {
			...
			try {
				...
				console.error(
					'Failed to fetch recent matches (fallback):',
					fallbackError
				)
			}
		}
	...
	let faceitURL =
		playerData.faceit_url ||
		`https://www.faceit.com/en/players/${playerData.nickname}`

	// Заменяем {lang} шаблон на реальный язык
...
export async function POST(request: NextRequest) {
	try {
		...
		if (steamId) {
			...
			try {
				const response = await axiosInstance.get(
					`${FACEIT_API_BASE}/players?nickname=${encodeURIComponent(
						trimmedInput
					)}`,
					{
						headers: getApiHeaders()
